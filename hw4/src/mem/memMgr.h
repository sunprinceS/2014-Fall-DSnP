...
// (ok)TODO: Define them by SIZE_T and/or SIZE_T_1 MACROs.
//
// To promote 't' to the nearest multiple of SIZE_T; 
// e.g. Let SIZE_T = 8;  toSizeT(7) = 8, toSizeT(12) = 16,toSizeT(8)=8 //???
#define toSizeT(t)      (((t-1)/SIZE_T)+1)*SIZE_T  // TODO(ok)
//
// To demote 't' to the nearest multiple of SIZE_T
// e.g. Let SIZE_T = 8;  downtoSizeT(9) = 8, downtoSizeT(100) = 96
#define downtoSizeT(t)  ((t-1)/SIZE_T)*SIZE_T   // TODO(ok)
...
class MemRecycleList
{
   ...
   void  pushFront(T* p) {
	  ...
	  _first = p;
	  /****IMPORTANT PART****///???
	  //turn p to the ptr type (size_t*)
	  void* tmp = (size_t*)nextRecord;
	  *((size_t*)p) = (size_t) tmp;//store in the prev 4or8 byte
   ...
   void reset() {
      // TODO (ok) 
	   ...
	   if(_nextList != NULL)
		  _nextList->reset();
	  //reset and check data member
	   //assert(_first == NULL && _nextList == NULL);
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO(ok)
	  //remove block except for first block
	  //reset recycle list
	  ...
	  //reallocate the first block or not
	  if(b != 0 && b != _blockSize) //different size
	  {
		  _blockSize = b;
		  delete _activeBlock;
		  _activeBlock = new MemBlock<T>(0, _blockSize);
   ...
   void  freeArr(T* p) {
	  ...
	  // ///??? 
      size_t n = *(size_t*)(p); //have been tested in newOp.cpp
      #ifdef MEM_DEBUG
...
private:
   ...
   size_t getArraySize(size_t t) const {
      assert(t % SIZE_T == 0);//have been TOSIZE
      assert(t >= S);
      // TODO (ok)
	  t = ((t-SIZE_T)/S); 
	 return  t;
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO(ok)
	  MemRecycleList<T>* curLs = &(_recycleList[m]);//???
	  MemRecycleList<T>* prevLs;
   	  ...
   	  while(curLs != NULL)
	  {
		  ...
		  curLs = curLs->getNextList();
	  }

	  assert(curLs == NULL && prevLs->_arrSize!=n);
	  //create a new recycleList
	  MemRecycleList<T>* newList = new MemRecycleList<T>(n);
	  prevLs->setNextList(newList);
      curLs = prevLs -> getNextList();
	  return curLs;
   }
   ...
   T* getMem(size_t t) {
	  ...
	  //search recycle_list
	  MemRecycleList<T>* recycleLs = getMemRecycleList(getArraySize(t));
	  // can't recycyle
	  if(recycleLs->numElm() == 0)
	  {	
		  if(_activeBlock->getMem(t,ret) == false) //not big enough
		  {
			  if (_activeBlock->getRemainSize() == sizeof(T))   //revyvle to [0]
			  {
				  getMemRecycleList(0)->pushFront((T*)(_activeBlock->_ptr));
				  #ifdef MEM_DEBUG
				  cout <<"Recycling "<< ret <<" to _recycleList[ "<< 0 << "]\n";
				  #endif // MEM_DEBUG
			  }
			  else if (_activeBlock->getRemainSize() > sizeof(T))
			  {
				  size_t n = (_activeBlock->getRemainSize()-SIZE_T)/S;
				  getMemRecycleList(n)->pushFront((T*)(_activeBlock->_ptr));
				  #ifdef MEM_DEBUG
				  cout <<"Recycling "<< ret <<" to _recycleList[ "<< n << "]\n";
				  #endif // MEM_DEBUG
			  }
			  else;
			  MemBlock<T>* nextBk = _activeBlock;
			  _activeBlock = new MemBlock<T>(nextBk,_blockSize);
			  #ifdef MEM_DEBUG
	  ...
	  {
		  ret = recycleLs->popFront();
		  #ifdef MEM_DEBUG
		  cout << "Recycled from _recycleList["               \
			   << recycleLs->_arrSize << "]..." << ret << endl;
		  #endif // MEM_DEBUG
	  }
      ...
      //    Note: recycle to the asiggest array index as possible???
